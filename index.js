...
================================= */
const sizes = {
  short:  { instruction: "Коротке резюме (~3 речення), але без втрати ключових фактів.", maxTokens: 600 },
  medium: { instruction: "Повне резюме (~6 речень) з усіма ключовими фактами.", maxTokens: 1000 },
  long:   { instruction: "Максимально повне резюме (~10 речень) без втрати деталей.", maxTokens: 1600 }
};
...
ЗАВДАННЯ:
Скороти текст до повного, точного резюме.

КРИТИЧНО ВАЖЛИВО:
- збережи всі ключові факти, характеристики і можливості
- не пропускай жоден блок (технічні характеристики, звук, геймінг, порівняння, дата, ціни)
- відповідь має бути повністю завершеною

РОБОТА З ЧИСЛАМИ:
- всі числа, дати, відсотки, валюти копіюй без змін
- не змінюй числа (наприклад 2030 на 2025)
- не вигадуй відсотки або символи
- якщо є число — залиш його як є

ФОРМУВАННЯ ТЕКСТУ:
- скорочуй текст, видаляючи зайве
- використовуй частини оригінальних речень
- можна перефразовувати, але без зміни фактів
- не роби список характеристик — це має бути звичайний текст

ЗАБОРОНЕНО:
- вигадувати
- змінювати числа
- пропускати важливі частини тексту
- обривати відповідь

КОНТРОЛЬ ЯКОСТІ (обов'язково):
Перед завершенням перевір:
- чи є всі блоки (характеристики, порівняння, функції, дата)
- чи не загублені числа
- чи текст завершений
